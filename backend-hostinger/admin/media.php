...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $stmt->execute([$info['original_name'], $info['stored_name'], $info['mime_type'], $info['size'], $info['path'], $info['public_url'], $_POST['alt_text'] ?? '', $_POST['usage_key'] ?? '']);
...
<table>
  <tr><th>Preview</th><th>Name</th><th>Usage</th><th>Public URL</th><th>Date</th></tr>
  <?php foreach ($rows as $row): ?>
    <tr>
      <td><img class="thumb" src="<?= e($row['public_url']) ?>" alt=""></td>
      <td><?= e($row['original_name']) ?><br><span class="muted"><?= e($row['mime_type']) ?> · <?= e((string)$row['size']) ?> bytes</span></td>
